...
int main(int argc, char **argv)
{
	...
	// Check arguments
	if (argc != 6) {
		fprintf(stderr, "Wrong Input! Enter desired core ID, slice ID, N of extra addresses (for the monitoring set to incur LLC misses), set ID, and channel interval!\n");
		printf("Enter: %s <core_ID> <slice_ID> <N of extra addresses> <set_ID> <interval>\n", argv[0]);
		exit(1);
	}
